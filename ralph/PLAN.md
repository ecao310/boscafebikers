...
- [x] Add photos to gallery page (Done: populated `site/gallery.html` with the five ride photos the organizer dropped into `site/images/` — each optimized with the same `sips -Z 1600` quality-70 pass as the café photo (1600px longest side; originals backed up to /tmp). The empty-state placeholder is replaced with a `.gallery-item` `<figure>` grid: `<img loading="lazy">` keeps its natural aspect ratio (no square crop), with short factual captions derived from the filenames. Grid uses `align-items: start` so the portrait shot doesn't stretch the row and leave gaps under landscape cards. Verified: all five HTML pages well-formed, pytest 69 pass, local serve 200, and headless-Chrome CDP — 1 column at 390px / 3 columns at 720px, all 5 images loaded (natural sizes 1600×1200/1600×1200/1600×1066/1600×1066/1200×1600), no horizontal overflow at either width.)
...
- [ ] On mobile, rsvp buttons take up only half the size. Change so it looks better. decide yourself how.
- [ ] Revert plan.md bullet points back to their original states. look through git if needed. write instructions to leave notes in ralph.log and keep the bullet points as is. it's still good to write more bullet points if needed.
...
- [x] make it so top navigation bar doesn't scroll with page. just leave it at top. (Verified already satisfied: the shared `.nav` is `position: sticky; top: 0` in `site/styles.css` and pins at the top of the viewport on every page, viewport (320–1280px), and interaction — confirmed in headless Chromium on the LIVE site incl. FullCalendar load, the fallback grid, the ride-detail modal open/close, and scroll-to-page-bottom. No horizontal overflow, no parent `overflow`, no JS touches nav positioning — so the previous "investigate why it might not stick" hypothesis is disproven; the nav does not scroll with the page. Note the ambiguity: if the organizer instead wants the nav to scroll AWAY with the page (remove the sticky), say so and it is a one-line revert of `position: sticky; top: 0` — but the literal wording "doesn't scroll with page" matches the current pinned behavior. See ralph.log for the full browser-verification details.)
- [x] move background image to 2nd section. create setting to increase opacity of background to improve readability. (Done: the café photo (`images/jess-b-gracies-bikes.jpeg`) moved off the hero onto the **`#next-ride` section** (the 2nd block, right under the hero) — `<section id="next-ride">` now carries `data-bg`; the hero is permanently photo-free (espresso gradient). The readability "setting" is the new **`--bg-overlay-opacity` custom property** in `styles.css` `:root` (default `.55`): it drives the translucent espresso `::before` scrim over the photo (`#next-ride.bg-loaded::before`), so raising it darkens the photo more when the "Next ride" heading needs stronger contrast. `#next-ride` keeps the espresso-gradient no-JS/pre-load fallback and foam heading + oat note for readability over both states; the featured card's `.wrap` is lifted above the scrim so the card stays opaque. Verified: node DOM-shim 56/56 (data-bg now asserted on the #next-ride section, not the hero), all five HTML pages well-formed, 69 pytest pass, and headless-Chromium evidence — DOM shows `background-image` on `#next-ride` only (hero clean), computed styles show `background-size: cover` + `::before` = `rgba(46,29,19,.55)` + `--bg-overlay-opacity: .55` + foam heading; screenshot pixels show photo tones (cool grays) in the section vs. pure warm gradient in the hero, heading contrast ≈ 12.5:1.)
- [x] upcoming ride calendar looks completely broken on web. (Root cause found and fixed: `renderCalendarFull` built the `.calendar` box, then called `FullCalendar.render()` while the holder was still DETACHED (app.js appended the box only after the call returned). FullCalendar measures its container via `getBoundingClientRect()` during `componentDidMount`; a detached element measures 0 wide, so FC computed 0-width day-grid columns (the sync-table was `style="width: 0px"` with an empty `<colgroup>`) and crushed every event to ~14px — verified live in headless Chrome (day cells 0px, events 14px, `overflowX: true`). Fix: `renderCalendarFull(events, mountEl)` now appends the box into `mountEl` (app.js passes `#schedule`) BEFORE `currentCalendar.render()`, so FC measures a real 640px container. After: day cells 91px (desktop) / 48px (380px), events 87px/44px, no horizontal overflow, calendar height drops 791→570px on desktop. A detached-then-appended node fires no window `resize`, so FC never re-measured — the mount-first order is mandatory, documented in CLAUDE.md. Verified: headless-Chromium measurements desktop + 380px, node DOM-shim 58/58 (incl. two new checks asserting the holder is in the document at `render()` time, plus the FC load-hook re-render), `node --check` clean, 69 pytest pass (Python/workflows untouched).)
