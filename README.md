...
- Ride data: `site/events.json` (upcoming) + `site/events-past.json` (archive), generated by `scripts/sync.py`
...
```
Partiful ICS feed  →  scripts/sync.py  →  site/events.json       →  site/index.html
   (repo secret)       (one process:    →  site/events-past.json     (fetch + render)
                        fetch, archive, →  site/cafe-points.json
                        backfill,       →  site/maps/*.svg
                        geocode, draw,  →  site/rides.ics
                        publish, export)   (committed by CI)
```
...
leaves the repo untouched and nothing is committed. Each step is still a
...
| `.github/actions/sync-branch/` | The composite action both sync jobs share: tests → `sync.py` → commit if changed |
